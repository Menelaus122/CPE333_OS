// Build PS06/docs/PS06_Report.docx from PS06/docs/PS06.tex.
//
// The report is written once, in LaTeX. This script reads that same file and renders the
// Word version from it, so the two deliverables cannot drift apart. It understands only the
// constructs PS06.tex actually uses: section/subsection/subsubsection, paragraphs with
// \code \textbf \textit, itemize/enumerate, table+tabular, figure+includegraphics and
// lstlisting. The title page is skipped and rebuilt here, since LaTeX and Word lay a cover
// page out very differently.
//
// Usage: node PS06/tools/build_report.js
const fs = require('fs');
...
const TEX = path.join(ROOT, 'docs', 'PS06.tex');
const OUT = process.env.REPORT_OUT || path.join(ROOT, 'docs', 'PS06_Report.docx');
...
const listItem = (tex, ref) => new Paragraph({
  ...
  if (trimmed.startsWith('\\begin{itemize}') || trimmed.startsWith('\\begin{enumerate}')) {
    ...
    for (i++; i < lines.length && !lines[i].trim().startsWith(close); i++) {
      ...
      if (t.startsWith('\\begin{lstlisting}')) {          // a code block inside a list item
        if (item !== null) { out.push(listItem(item, ref)); item = null; }
        const code = [];
      ...
      if (t.startsWith('\\item')) {
        if (item !== null) out.push(listItem(item, ref));
...
function cover() {
  ...
  return [
    ...
    center('PS06: Virtual Memory', 36, true, 640),
...
const doc = new Document({
  creator: 'CPE 333 Problem Session 6 group',
  title: 'CPE 333 Problem Session 6: Virtual Memory',
  styles: {
